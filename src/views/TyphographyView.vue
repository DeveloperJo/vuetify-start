<template>
  <v-container>
    <v-card>
      <v-card-title> Typhography </v-card-title>
      <v-container class="grey lighten-3">
        <v-card class="pa-3" outlined>
          <v-card class="pa-3 text-h2 primary lighten-1"> 콘텐츠 </v-card>
          <v-card class="pa-3 text-h2 secondary darken-1"> 콘텐츠 </v-card>
          <v-card class="pa-3 text-h2 error"> 콘텐츠 </v-card>
          <v-card class="pa-3 text-h2 primary--text"> 콘텐츠 </v-card>
          <v-card class="pa-3 text-h2 secondary--text"> 콘텐츠 </v-card>
          <v-card class="pa-3 text-h2 error--text"> 콘텐츠 </v-card>
          <p class="text-h1">Heading 1</p>
          <p class="text-h2">Heading 2</p>
          <p class="text-h3 text-sm-subtitle-1">Heading 3</p>
          <p class="display-4">display-4</p>
          <p class="font-weight-black">Black text</p>
          <p class="font-weight-thin">Thin text</p>
          <p class="text-justify">
            Morbi mattis ullamcorper velit. Donec orci lectus, aliquam ut,
            faucibus non, euismod id, nulla. Fusce convallis metus id felis
            luctus adipiscing. Aenean massa. Vestibulum purus quam, scelerisque
            ut, mollis sed, nonummy id, metus. Nulla consequat massa quis enim.
            Praesent venenatis metus at tortor pulvinar varius. Donec venenatis
            vulputate lorem. Phasellus accumsan cursus velit. Pellentesque ut
            neque.
          </p>
          <p class="text-right text-sm-left">
            Right aligned text on all viewport sizes.
          </p>
          <p class="text-decoration-none">decoration none</p>
          <p class="text-decoration-line-through">line through</p>
          <p class="text-decoration-underline">underline through</p>
          <!-- Heading 2
            Heading 3
            Heading 4
            Heading 5
            Heading 6
            Subtitle 1
            Subtitle 2
            Body 1
            Body 2
            BUTTON
            Caption
            OVERLINE -->
        </v-card>
      </v-container>
    </v-card>
  </v-container>
</template>

<script>
export default {};
</script>

<style></style>
